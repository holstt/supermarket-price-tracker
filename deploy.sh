...
# TODO: Maybe seperate this into two scripts, such that config is passed as arguments to the deploy script.
...
##### USER INPUT ######
#######################
PROJECT_DIR_NAME="supermarket-price-tracker" # Should be the name of the root dir of project on both local machine and remote server. Rsync will create the directory on the remote server if it doesn't exist
REMOTE_SERVER="vm1" # Shell alias for the remote server or use username@remote_server_address
REMOTE_COMMAND="export CONFIG_PATH=../config.json && cd $PROJECT_DIR_NAME/docker && docker-compose up --build" # Command that will run the program on the remote server
...
RSYNC_COMMAND="-m -av --progress "${source_files[@]}" "${exclude_pattern[@]}" . $DEST"
