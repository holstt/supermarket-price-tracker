...
class RemaScraper(Scraper):
    ...
    def __init__(self, data_storage: DataStorage, client: RemaClient):
    ...
    async def scrape(self):
        ...
        departmentDtos = self._client.fetch_departments()

        total_departments = len(departmentDtos)
        total_categories = sum(
            len(department.categories) for department in departmentDtos
        )
        ...
        await self.scrape_departments(departmentDtos)
    ...
    async def scrape_departments(self, departmentDtos: list[RemaDepartmentDto]):
    ...
    async def _scrape_categories(self, department: RemaDepartmentDto):
        for idx, category in enumerate(department.categories):
            ...
            categoryJson = self._client.fetch_products_json(category.id)

            file_name = f"dep_{department.id}_cat_{category.id}.json"
            logger.info(f"-- Saving to file: {file_name}")
            self._data_storage.save_data(file_name, categoryJson)
            await self._wait()
